...
BEGIN
  ...
  UPDATE public.at_risk_flags
   ...
   WHERE student_id = NEW.student_id
     ...
     AND (
       (_win_from IS NOT NULL AND flagged_at::date < _win_from) OR
       (_win_to IS NOT NULL AND flagged_at::date > _win_to)
     );
  ...
  IF _paid_inv + _att + _paid_pay + _unpaid_inv > 0 THEN
    ...
    LOOP
      INSERT INTO public.notification_queue(
        ...
        _admin.user_id,
        'admin',
        'assignment_window_changed',
        'Assignment accountability window changed',
        'An assignment status or date change affected accountability records.',
        _payload,
        'pending'
      );
    END LOOP;

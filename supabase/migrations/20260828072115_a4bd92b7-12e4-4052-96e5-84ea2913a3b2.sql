...
BEGIN
  ...
  IF _period_type = 'permanent' THEN
    ...
    -- Close out an earlier period that started before this one. A period that
    -- starts on the SAME date is simply superseded by the new row (the resolver
    -- prefers the most recently created period), so re-editing the same start
    -- date is allowed.

...
BEGIN
  ...
  -- Resolve assignment (either direct or first active by student)
  ...
  IF a.id IS NOT NULL AND a.status IS NOT NULL AND a.status <> 'active' THEN
    RETURN;
  END IF;

  -- Start = later of plan.effective_from and assignment.effective_from_date
  eff_start := GREATEST(COALESCE(p.effective_from, a.effective_from_date, date_trunc('month', now())::date),
                        COALESCE(a.effective_from_date, p.effective_from, date_trunc('month', now())::date));
  IF eff_start IS NULL THEN
    eff_start := date_trunc('month', now())::date;
  END IF;

  eff_end := CASE WHEN a.status IS NULL OR a.status = 'active' THEN NULL ELSE a.effective_to_date END;
...
CREATE OR REPLACE FUNCTION public.trg_auto_generate_plan_invoices()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  IF (TG_OP = 'INSERT' AND NEW.is_active) OR
     (TG_OP = 'UPDATE' AND (
        ...
        OR NEW.net_recurring_fee IS DISTINCT FROM OLD.net_recurring_fee
        OR NEW.currency IS DISTINCT FROM OLD.currency
        OR NEW.effective_from IS DISTINCT FROM OLD.effective_from
        OR NEW.assignment_id IS DISTINCT FROM OLD.assignment_id
     ))
  THEN
    PERFORM public.auto_generate_plan_invoices(NEW.id);
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS auto_generate_plan_invoices_trigger ON public.student_billing_plans;
CREATE TRIGGER auto_generate_plan_invoices_trigger
AFTER INSERT OR UPDATE ON public.student_billing_plans
FOR EACH ROW EXECUTE FUNCTION public.trg_auto_generate_plan_invoices();

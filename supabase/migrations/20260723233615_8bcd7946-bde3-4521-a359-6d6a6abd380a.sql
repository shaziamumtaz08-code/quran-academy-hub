...
-- 1) For each (student_id, billing_month) with a paid row, drop all other unpaid pending rows.
DELETE FROM public.fee_invoices fi
 WHERE fi.status = 'pending'
   ...
   AND EXISTS (
     SELECT 1 FROM public.fee_invoices fi2
      WHERE fi2.student_id = fi.student_id
        AND fi2.billing_month = fi.billing_month
        AND fi2.id <> fi.id
        AND fi2.status = 'paid'
   );

-- 2) For (student_id, billing_month) with multiple pending rows, keep the oldest, drop the rest.
DELETE FROM public.fee_invoices fi
 WHERE fi.status = 'pending'
   AND COALESCE(fi.amount_paid, 0) = 0
   AND COALESCE(fi.is_archived, false) = false
   AND fi.id NOT IN (
     SELECT DISTINCT ON (student_id, billing_month) id
       FROM public.fee_invoices
      WHERE status = 'pending'
        AND COALESCE(amount_paid, 0) = 0
        AND COALESCE(is_archived, false) = false
      ORDER BY student_id, billing_month, created_at ASC
   );

-- 3) Backfill assignment_id where NULL — but only where it won't collide with the unique key.
UPDATE public.fee_invoices fi
   SET assignment_id = sbp.assignment_id
  FROM public.student_billing_plans sbp
 WHERE fi.plan_id = sbp.id
   AND fi.assignment_id IS NULL
   AND sbp.assignment_id IS NOT NULL
   AND NOT EXISTS (
     SELECT 1 FROM public.fee_invoices fi2
      WHERE fi2.assignment_id = sbp.assignment_id
        AND fi2.billing_month = fi.billing_month
        AND fi2.id <> fi.id
   );

-- 4) Harden the generator: existence check by (plan_id, billing_month).
  ...
  WHILE m <= horizon LOOP
    ...
    IF active_days > 0 THEN
      ...
      SELECT * INTO existing_inv FROM public.fee_invoices
        WHERE plan_id = p.id AND billing_month = month_key

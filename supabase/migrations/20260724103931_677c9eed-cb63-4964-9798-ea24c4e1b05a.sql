...
-- 1. Patch generator: bail out for archived students or plans with no assignment link
...
BEGIN
  ...
  -- GUARD: skip archived students; void any future unpaid invoices
  ...
  -- GUARD: no assignment at all → nothing to bill; void pending invoices
  ...
  m := date_trunc('month', GREATEST(eff_start, (date_trunc('month', now()))::date))::date;
  ...
  WHILE m <= horizon LOOP
    ...
    ELSIF existing_inv.status = 'pending' AND COALESCE(existing_inv.amount_paid,0) = 0 AND existing_inv.voided_at IS NULL THEN
      UPDATE public.fee_invoices
         SET amount = prorated,
...
UPDATE public.fee_invoices
   SET voided_at = now(),
       void_reason = 'student_archived',
       updated_at = now()
 WHERE student_id = '01629ba9-092b-473a-b6d6-bbe2e1b6631e'
   AND status = 'pending'
   AND COALESCE(amount_paid,0) = 0
   AND voided_at IS NULL;
...
UPDATE public.student_billing_plans
   SET is_active = false,
       ...
       change_reason = COALESCE(change_reason, 'student_archived'),
       updated_at = now()
 WHERE student_id = '01629ba9-092b-473a-b6d6-bbe2e1b6631e'
   AND is_active = true;

  ...
  const { data: recent = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return messages.map((message) => ({
        ...message,
        senderName: profileMap[message.sender_id] || 'Unknown',
        groupName: groupMap[message.group_id] || 'Chat',
      }));
    },
    enabled: !!user?.id,
    ...
    >
      ...
      {isLoading ? (
        <div className="space-y-2">
          {[0, 1, 2].map((index) => <Skeleton key={index} className="h-12 rounded-lg" />)}
        </div>
      ) : recent.length === 0 ? (
        <p className="py-4 text-center text-sm" style={{ color: palette.textMuted }}>
              ...
              <div key={message.id} className="flex items-center gap-3 rounded-lg px-2 py-2 transition-colors hover:bg-white/5">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="truncate text-sm font-semibold" style={{ color: palette.text }}>
                      {message.senderName}
                    </span>
                ...
                <span className="shrink-0 text-[11px]" style={{ color: palette.textMuted }}>
                  {formatCommTime(message.created_at)}
                </span>
              </div>
            );
          })}
  ...
  const contentMap: Record<string, React.ReactNode> = useMemo(() => ({
    'academy-chat': (
      <div className="space-y-4">
        <RecentActivityStrip />
        <Suspense fallback={<Loading />}><GroupChat /></Suspense>
      </div>
    ),
    ...
    'work-hub': <Suspense fallback={<Loading />}><WorkHub /></Suspense>,
  }), []);

  return (
    <HubPageShell
      title="Communication"
      subtitle="Chat, WhatsApp, notifications, classroom operations, and work hub"
      kpis={[
        { label: 'Unread Chats', value: counts?.chat, loading: isLoading },
        { label: 'WhatsApp Pending', value: counts?.whatsapp, tone: 'warning', loading: isLoading },
        { label: 'Open Tickets', value: counts?.tickets, tone: 'danger', loading: isLoading },
      ]}
      tabs={[
        { label: 'Academy Chat', value: 'academy-chat' },
        { label: 'WhatsApp Inbox', value: 'whatsapp' },
        { label: 'Notifications', value: 'notifications' },
        { label: 'Zoom', value: 'zoom' },
        { label: 'Work Hub', value: 'work-hub' },
      ]}
      defaultTab="academy-chat"
      content={contentMap}
    />
  );
}

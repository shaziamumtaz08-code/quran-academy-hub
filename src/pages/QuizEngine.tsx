...
export default function QuizEngine() {
  ...
  // Background pipeline: raw files kept for server-side extraction of large books
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Big books are queued for the background pipeline instead of being parsed in the browser
    const heavy = picked.some(f => f.size > 4 * 1024 * 1024);
    if (backgroundMode || heavy) {
      setBackgroundMode(true);
      setRawFiles(prev => [...prev, ...picked].slice(0, 5));
      setUploadedFiles(prev => [...prev, ...picked.map(f => ({ name: f.name, text: '' }))].slice(0, 5));
      toast({
        title: 'Queued for background processing',
        description: 'Large source detected — extraction will run on the server after you create the bank.',
      });
  ...
  const createBank = useMutation({
    mutationFn: async () => {
      ...
      // 2a. Large sources → queue the background extraction + generation pipeline
      if (backgroundMode && rawFiles.length) {
        await startQuizExtractionJob({
          quizBankId: bank.id,
          files: rawFiles,
          params: {
            language: form.language,
        ...
        return { background: true, count: 0 };
      }

      // 2b. Small sources → generate inline
    ...
    onSuccess: (data: any) => {
      ...
      toast(data?.background
        ? { title: 'Processing in the background', description: 'You can close this page — progress is shown on the Quiz Banks tab.' }
        : { title: 'Quiz Bank Created', description: `${data.count} questions generated by AI` });
    },

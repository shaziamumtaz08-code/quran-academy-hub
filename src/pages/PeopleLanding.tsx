...
export default function PeopleLanding() {
      ...
      if (courseIds.length > 0) {
        ...
        if (classIds.length > 0) {
          ...
          (classStaff || []).forEach((row: any) => {
            if (row.user_id && (row.staff_role === 'teacher' || !row.staff_role)) {
  ...
  const banner = !!dupCount && dupCount > 0 ? (
  ...
  return (
    <HubPageShell
      title="People"
      subtitle="Manage teachers, students, staff access, and leads"
      banner={banner}
      kpis={[
        ...
        { label: 'Open Leads', value: counts?.leads, tone: 'warning', loading: isLoading },
      ]}
      tabs={[

...
import type { RoleSalaryRow } from "@/pages/SalaryEngine";
...
export function SalarySheetDialog({
  ...
  const dotColor = (status: string) => {
    switch (status) {
      ...
      case "holiday": return "bg-sky-400";
      ...
      <DialogContent className="max-w-5xl max-h-[95vh] p-0 gap-0 overflow-hidden w-[95vw] sm:w-auto print:max-w-none print:max-h-none print:shadow-none print:border-none">
        <ScrollArea className="max-h-[95vh]">
          <div className="salary-sheet-print">
                    ...
                    <div className="bg-white/10 rounded-lg p-2.5 mt-1 space-y-1">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-0.5 text-xs sm:text-sm">
                        {teacherProfile?.bank_account_title && (
                          <div><span className="opacity-60">Title:</span> {teacherProfile.bank_account_title}</div>
                        )}
                        {teacherProfile?.bank_name && (
                          <div><span className="opacity-60">Bank:</span> {teacherProfile.bank_name}</div>
                        )}
                        {teacherProfile?.bank_account_number && (
                          <div><span className="opacity-60">A/C #:</span> {teacherProfile.bank_account_number}</div>
                        )}
                        {teacherProfile?.bank_iban && (
                          <div><span className="opacity-60">IBAN:</span> {teacherProfile.bank_iban}</div>
                        )}
                      </div>
                    </div>
                  )}
                  ...
                  {!isTeacherView && hasStudents && (
                    <div className="bg-white/10 rounded-lg p-2.5 space-y-1">
                      ...
                      <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-xs sm:text-sm">
                        <span className="flex items-center gap-1">
                          <span className="w-2 h-2 rounded-full bg-emerald-400 inline-block shrink-0" /> {teacherAttendance.present} Present
                        </span>
                        <span className="flex items-center gap-1">
                          <span className="w-2 h-2 rounded-full bg-red-400 inline-block shrink-0" /> {teacherAttendance.absent} Absent
                        </span>
                        <span className="flex items-center gap-1">
                          <span className="w-2 h-2 rounded-full bg-amber-400 inline-block shrink-0" /> {teacherAttendance.leave} Leave
                        </span>
                        <span className="flex items-center gap-1">
                          <span className="w-2 h-2 rounded-full bg-gray-300 inline-block shrink-0" /> {teacherAttendance.notMarked} N/M
                        </span>
                      </div>
                    </div>
                  )}
            ...
            {/* ═══ ROLE-BASED SALARY SECTION ═══ */}
            {hasRoleSalaries && (
              <div className="p-3 sm:p-6 space-y-3">
                <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                  <Briefcase className="h-3.5 w-3.5" /> Role-Based Salary
                </p>
                {(teacher.roleSalaries || []).map((rs) => {
                  const finalAmt = editRoleAmounts[rs.staffSalaryId] ?? rs.proratedAmount;
                  return (
                    <div key={rs.staffSalaryId} className="border border-border rounded-xl bg-card shadow-sm overflow-hidden">
                      <div className="grid grid-cols-10 gap-3 items-center px-4 py-3">
                        <div className="col-span-3">
                          ...
                          <p className="text-[10px] text-muted-foreground mt-0.5">
                            {rs.activeDays}/{rs.totalDays} days
                            {rs.activeDays < rs.totalDays && ' (pro-rated)'}
                          </p>
                        </div>
                        ...
                        <div className="col-span-2 text-right">
                          <span className="text-sm font-medium tabular-nums">PKR {rs.monthlyAmount.toFixed(2)}</span>
                        ...
                        <div className="col-span-3 text-right">
                          {canEditCalculations ? (
                            <Input
                              ...
                              value={finalAmt}
                              onChange={(e) => onEditRoleAmount?.(rs.staffSalaryId, parseFloat(e.target.value) || 0)}
                            />
                          ) : (
                            <span className="text-sm font-semibold tabular-nums">PKR {finalAmt.toFixed(2)}</span>
                          )}
                        </div>
                      </div>
                    </div>
                ...
                {hasStudents && hasRoleSalaries && (
                  <div className="text-right text-xs text-muted-foreground">
                    Role subtotal: <span className="font-semibold">PKR {roleBase.toFixed(2)}</span>
                  </div>
                )}
              </div>
            )}
            ...
            {hasStudents && (
              <div className="p-3 sm:p-6 space-y-3">
                ...
                {hasStudents && hasRoleSalaries && (
                  <div className="text-right text-xs text-muted-foreground">
                    Teaching subtotal: <span className="font-semibold">PKR {teachingBase.toFixed(2)}</span>
                  </div>
                )}
              </div>
            )}

            {/* No data message for staff-only with no role salaries */}
            {!hasStudents && !hasRoleSalaries && (
              <div className="text-center py-12 text-muted-foreground">No salary data for this period</div>
            )}
                ...
                <div className="space-y-2">
                  {teacher.extraClassAmount > 0 && (
                    <div className="flex justify-between items-center py-2 px-3 bg-emerald-50 rounded-lg">
                      <div><p className="text-sm font-semibold text-foreground">Extra Classes</p></div>
                  ...
                  {adjustments.map((adj) => (
                    ...
                    >
                      <div className="min-w-0 flex-1 mr-3">
                        <p className="text-sm font-semibold text-foreground capitalize">{adjustmentTypeLabel(adj.adjustment_type)}</p>
            ...
            <div className="border-t-2 border-border bg-muted/30 px-4 sm:px-6 py-4 sm:py-5">
                            ...
                            <div className="flex-1">
                              {receiptUrls[idx] ? (
                                <div className="flex items-center gap-2 p-2 bg-muted rounded-lg">
                                  ...
                                  <Button type="button" size="sm" variant="ghost" className="h-6 w-6 p-0 text-destructive hover:text-destructive" onClick={() => removeReceipt(idx)}>
              ...
              {isPartiallyPaid && !isLocked && (
                <div className="space-y-3 print:hidden">
                  ...
                  {existingInvoiceNumber && <p className="text-xs text-muted-foreground">Invoice: {existingInvoiceNumber}</p>}

                  {!isTeacherView && (
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                      <Button
                        className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white"
                        onClick={() => onMarkPaid("partial", "Marked fully paid", displayInvoice, receiptUrls, grandNet)}
                        disabled={isPayingPending}
                      >
                        {isPayingPending && <Loader2 className="h-4 w-4 animate-spin mr-1" />}
                        <CheckCircle className="h-4 w-4 mr-1.5" /> Mark Fully Paid
                    ...
                    <div className="p-3 border border-border bg-muted/30 rounded-lg space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1">
                          ...
                          <Input type="number" placeholder="0.00" value={topUpAmount} onChange={(e) => setTopUpAmount(e.target.value)} className="h-8 text-sm" />
                        </div>
                        <div className="space-y-1">
                      ...
                      <div className="space-y-1">
                        ...
                        <Textarea placeholder="Top-up reason or notes..." value={topUpNotes} onChange={(e) => setTopUpNotes(e.target.value)} className="text-sm min-h-[60px]" />
                      </div>
                      <div className="space-y-1">
                        ...
                        {topUpReceipts[0] ? (
                          <div className="flex items-center gap-2 p-2 bg-background rounded-lg">
                            ...
                            <Button type="button" size="sm" variant="ghost" className="h-6 w-6 p-0 text-destructive hover:text-destructive" onClick={() => setTopUpReceipts([])}>
                              <X className="h-3.5 w-3.5" />
                            </Button>
                          </div>
                        ) : (
                  ...
                  {isTeacherView && receiptUrls.length > 0 && (
                    <div className="space-y-1">
                      {receiptUrls.map((url, idx) => <AttachmentPreview key={idx} url={url} />)}
                    </div>
                  )}
                </div>
              )}
              ...
              {canRevert && (
                <div className="mt-4 pt-3 border-t border-border print:hidden">
                  <Button variant="outline" className="text-amber-600 border-amber-300 hover:bg-amber-50 hover:text-amber-700" onClick={onRevert}>
                    <RotateCcw className="h-4 w-4 mr-1.5" /> Revert to Draft
                  </Button>
                  <p className="text-[10px] text-muted-foreground mt-1">This will reset the salary back to draft status for editing.</p>
                </div>
              )}
            </div>
          </div>
...
function FeeBadge({ status }: { status: string }) {
  ...
  return <Badge className="bg-amber-50 text-amber-700 border-amber-200 text-[10px]"><Clock className="h-3 w-3 mr-0.5" />UNPAID</Badge>;
}

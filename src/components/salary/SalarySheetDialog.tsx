...
import {
  ...
  CheckCircle, XCircle, Clock, AlertCircle, Printer,
  User, Loader2
} from 'lucide-react';
                ...
                <div className="space-y-2 flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs sm:text-sm opacity-90">
                    {teacherProfile?.whatsapp_number && (
                      <span className="flex items-center gap-1"><Phone className="h-3 w-3 shrink-0" />{teacherProfile.whatsapp_number}</span>
                    )}
                    {(teacherProfile?.city || teacherProfile?.country) && (
                      <span className="flex items-center gap-1"><MapPin className="h-3 w-3 shrink-0" />{[teacherProfile.city, teacherProfile.country].filter(Boolean).join(', ')}</span>
                    )}
                    {teacherProfile?.email && (
                      <span className="flex items-center gap-1 break-all"><User className="h-3 w-3 shrink-0" />{teacherProfile.email}</span>
                    )}
                  </div>
                  {(teacherProfile?.bank_name || teacherProfile?.bank_account_number) && (
                    <div className="bg-white/10 rounded-lg p-2.5 mt-1 space-y-1">
                      <p className="text-[10px] uppercase tracking-wider opacity-60 flex items-center gap-1"><CreditCard className="h-3 w-3" />Bank Details</p>
            ...
            {(adjustments.length > 0 || teacher.extraClassAmount > 0) && (
              <div className="px-4 sm:px-6 pb-4">
                ...
                <div className="space-y-2">
                  ...
                  {adjustments.map(adj => (
                    <div key={adj.id} className={`flex justify-between items-start py-2 px-3 rounded-lg ${adj.adjustment_type === 'deduction' ? 'bg-red-50' : 'bg-emerald-50'}`}>

...
import { StatsRowCompact } from './StatsRowCompact';
...
export function ChildrenDashboardView({
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {children.map((c, idx) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-bold whitespace-nowrap border transition-colors ${idx === activeChildIdx ? 'bg-primary text-primary-foreground border-primary' : 'bg-card text-foreground border-border hover:bg-secondary'}`}
            >
              {c.full_name}
            </button>
          ))}
          ...
          <div className="bg-card rounded-md border border-border overflow-hidden divide-y divide-border">
            {child.recentLessons.map((lesson, idx) => (
              <div key={idx} className="px-3 py-2.5 flex items-center gap-2.5">
                <div className={`w-8 h-8 rounded-md flex items-center justify-center text-[10px] font-bold flex-shrink-0 ${lesson.status === 'present' ? 'bg-teal/10 text-teal' : 'bg-destructive/10 text-destructive'}`}>
  ...
  const rightContent = (
    <>
      <div className="bg-card rounded-md border border-border p-4 shadow-card">
      ...
      <div className="bg-card rounded-md border border-gold/20 p-3.5 shadow-card">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="bg-gold/10 text-gold border border-gold/15 rounded-md px-3 py-1.5 font-bold text-xs hover:opacity-90 transition-opacity"
          >
            View Fees →
          </button>
        </div>
      ...
      <button
        ...
        className="w-full bg-gradient-to-br from-primary/10 to-accent/10 rounded-md border border-primary/20 p-3.5 shadow-card flex items-center gap-3 hover:opacity-90 transition-opacity text-left"
      >
        <div className="w-10 h-10 rounded-md bg-primary/15 flex items-center justify-center text-primary shrink-0">

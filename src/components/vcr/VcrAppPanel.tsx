  ...
  return (
    <div className="flex shrink-0 gap-1.5">
      <button
        ...
        className="inline-flex h-7 items-center gap-1 rounded-full border border-vcr-chrome/20 px-2.5 text-[11px] text-vcr-chrome/75 hover:text-vcr-chrome"
      >
      ...
      <button
        ...
        className="inline-flex h-7 items-center gap-1 rounded-full border border-vcr-gold/50 bg-vcr-gold/15 px-2.5 text-[11px] text-vcr-gold"
      >
...
function Row({ title, subtitle, children }: { title: string; subtitle?: string; children: React.ReactNode }) {
  return (
    <li className="flex items-center gap-2 rounded-xl border border-vcr-chrome/10 bg-black/20 px-3 py-2">
      <span className="min-w-0 flex-1">
        <span className="block truncate text-sm text-vcr-chrome">{title}</span>
        {subtitle && <span className="block truncate text-[11px] text-vcr-chrome/45">{subtitle}</span>}
      </span>
      {children}
    </li>
  );
  ...
  if (app === 'drive' || app === 'youtube' || app === 'url') {
    ...
    return (
      <div className="space-y-3">
        <p className="text-sm text-vcr-chrome/60">
          {app === 'drive'
        ...
        <input
          ...
          placeholder="https://…"
          className="h-10 w-full rounded-xl border border-vcr-chrome/20 bg-black/25 px-3 text-sm text-vcr-chrome placeholder:text-vcr-chrome/35 focus:border-vcr-gold/60 focus:outline-none"
        />
        {link && !embed && (
          <p className="text-xs text-red-300">That link is not one we can open here. Check it and try again.</p>
        )}
  ...
  if (app === 'syllabus') {
    return (
      <div className="space-y-3">
        ...
        {docsLoading && <p className="text-sm text-vcr-chrome/50"><Loader2 className="inline h-3.5 w-3.5 animate-spin" /> Loading syllabus files…</p>}
        {docsError && <p className="text-sm text-red-300">Could not load syllabus files: {docsError}</p>}
        {!docsLoading && !docsError && syllabusDocs.length === 0 && (
          <p className="text-sm text-vcr-chrome/50">No syllabus files have been set for this class yet.</p>
        )}
        <ul className="space-y-1.5">
          {syllabusDocs.map((d) => (
  ...
  if (app === 'library') {
    return (
      <div className="space-y-3">
        ...
        {docsLoading && <p className="text-sm text-vcr-chrome/50"><Loader2 className="inline h-3.5 w-3.5 animate-spin" /> Loading the library…</p>}
        {docsError && <p className="text-sm text-red-300">Could not load the library: {docsError}</p>}
        {!docsLoading && !docsError && libraryDocs.length === 0 && (
          <p className="text-sm text-vcr-chrome/50">Nothing in the library is available to you yet.</p>
        )}
        <ul className="space-y-1.5">
          {libraryDocs.map((d) => (
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <p className="min-w-0 flex-1 text-sm text-vcr-chrome/60">
        ...
        {onUpload && (
          <button
            type="button"
            onClick={onUpload}
            className="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-full border border-vcr-chrome/20 px-3 text-xs text-vcr-chrome/70 hover:text-vcr-chrome"
          >
            <Upload className="h-3.5 w-3.5" /> Add file
          </button>
        )}
      </div>
      {resourcesLoading && <p className="text-sm text-vcr-chrome/50"><Loader2 className="inline h-3.5 w-3.5 animate-spin" /> Loading your files…</p>}
      {!resourcesLoading && mine.length === 0 && sharedWithMe.length === 0 && myDocs.length === 0 && (
        <p className="text-sm text-vcr-chrome/50">Nothing here yet. Add a file, or save something from the Library.</p>
      )}
      <ul className="space-y-1.5">
        {mine.map((r) => (

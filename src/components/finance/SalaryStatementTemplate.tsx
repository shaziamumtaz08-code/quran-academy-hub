import React from 'react';
import { format, parseISO } from 'date-fns';
import logoDark from '@/assets/logo-dark.jpg';

interface StudentRow {
  studentName: string;
  dateFrom: string;
  dateTo: string;
  payoutRate: number;
  payoutType: string;
  eligibleDays: number;
  totalDays: number;
  calculatedAmount: number;
  editedAmount: number | null;
}

interface RoleSalaryPrintRow {
  role: string;
  monthlyAmount: number;
  effectiveFrom: string;
  effectiveTo: string;
  activeDays: number;
  totalDays: number;
  proratedAmount: number;
  editedAmount: number | null;
}

interface Adjustment {
  adjustment_type: string;
  adjustment_mode?: string;
  amount: number;
  percentage_value?: number | null;
  resolved_amount?: number | null;
  reason: string | null;
}

interface SalaryStatementTemplateProps {
  teacherName: string;
  teacherId: string;
  email?: string | null;
  phone?: string | null;
  location?: string | null;
  bankName?: string | null;
  bankAccountTitle?: string | null;
  bankAccountNumber?: string | null;
  bankIban?: string | null;
  monthLabel: string;
  invoiceNumber: string;
  students: StudentRow[];
  roleSalaries?: RoleSalaryPrintRow[];
  extraClassAmount: number;
  adjustments: Adjustment[];
  baseSalary: number;
  additions: number;
  deductions: number;
  netSalary: number;
  paymentDate?: string | null;
  paymentMethod?: string | null;
  receiptUrl?: string | null;
  orgName?: string;
  orgLogo?: string | null;
}

const sectionTitle: React.CSSProperties = {
  fontSize: 9,
  textTransform: 'uppercase',
  letterSpacing: '0.15em',
  color: '#9ca3af',
  fontWeight: 700,
  marginBottom: 10,
  margin: 0,
  paddingBottom: 6,
  borderBottom: '1px solid #e5e7eb',
};

const mono: React.CSSProperties = { fontFamily: "'JetBrains Mono', monospace" };

export function SalaryStatementTemplate({
  teacherName, teacherId, email, phone, location,
  bankName, bankAccountTitle, bankAccountNumber, bankIban,
  monthLabel, invoiceNumber, students, roleSalaries = [], extraClassAmount,
  adjustments, baseSalary, additions, deductions, netSalary,
  paymentDate, paymentMethod, receiptUrl,
  orgName = 'Al-Quran Time Academy', orgLogo
}: SalaryStatementTemplateProps) {
  const logoSrc = orgLogo || logoDark;

  const additionAdjs = adjustments.filter(a => a.adjustment_type !== 'deduction');
  const deductionAdjs = adjustments.filter(a => a.adjustment_type === 'deduction');

  const formatAmount = (n: number) => n.toLocaleString(undefined, { minimumFractionDigits: 2 });

  return (
    <div style={{ width: 794, margin: '0 auto', background: '#fff', color: '#111827', fontFamily: "'Inter', 'Segoe UI', sans-serif", position: 'relative', overflow: 'hidden' }}>
      {/* Decorative top accent */}
      <div style={{ height: 6, background: 'linear-gradient(90deg, #0a192f 0%, #00a8e8 50%, #0a192f 100%)' }} />

      {/* ═══ HEADER ═══ */}
      <div style={{ padding: '28px 48px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <img src={logoSrc} alt="Logo" style={{ height: 56, width: 56, objectFit: 'contain', borderRadius: 8 }} />
          <div>
            <h1 style={{ fontSize: 20, fontWeight: 800, color: '#0a192f', margin: 0, letterSpacing: '-0.02em' }}>{orgName}</h1>
            <p style={{ fontSize: 10, color: '#00a8e8', letterSpacing: '0.2em', textTransform: 'uppercase', fontWeight: 700, margin: '4px 0 0' }}>Salary Statement</p>
          </div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <p style={{ fontSize: 18, fontWeight: 800, color: '#0a192f', margin: 0 }}>{monthLabel}</p>
          <p style={{ fontSize: 11, color: '#6b7280', margin: '2px 0 0' }}>Ref: {invoiceNumber}</p>
        </div>
      </div>

      <div style={{ margin: '0 48px', height: 1, background: 'linear-gradient(90deg, #e5e7eb, #d1d5db, #e5e7eb)' }} />

      {/* ═══ EMPLOYEE INFO ═══ */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, padding: '20px 48px' }}>
        <div>
          <p style={sectionTitle}>Employee Details</p>
          <p style={{ fontSize: 16, fontWeight: 700, color: '#0a192f', margin: '8px 0 0' }}>{teacherName}</p>
          <p style={{ fontSize: 11, color: '#6b7280', margin: '2px 0 0' }}>ID: {teacherId.substring(0, 8).toUpperCase()}</p>
          {email && <p style={{ fontSize: 11, color: '#6b7280', margin: '2px 0 0' }}>{email}</p>}
          {phone && <p style={{ fontSize: 11, color: '#6b7280', margin: '2px 0 0' }}>{phone}</p>}
          {location && <p style={{ fontSize: 11, color: '#6b7280', margin: '2px 0 0' }}>{location}</p>}
        </div>
        {(bankName || bankAccountNumber) && (
          <div>
            <p style={sectionTitle}>Bank Details</p>
            <div style={{ background: '#f8fafc', borderRadius: 8, padding: 12, border: '1px solid #e2e8f0', marginTop: 8 }}>
              {bankAccountTitle && <p style={{ fontSize: 12, margin: '0 0 3px' }}><span style={{ color: '#6b7280' }}>Title:</span> <strong>{bankAccountTitle}</strong></p>}
              {bankName && <p style={{ fontSize: 12, margin: '0 0 3px' }}><span style={{ color: '#6b7280' }}>Bank:</span> {bankName}</p>}
              {bankAccountNumber && <p style={{ fontSize: 12, margin: '0 0 3px' }}><span style={{ color: '#6b7280' }}>A/C #:</span> <strong style={mono}>{bankAccountNumber}</strong></p>}
              {bankIban && <p style={{ fontSize: 12, margin: 0 }}><span style={{ color: '#6b7280' }}>IBAN:</span> <span style={mono}>{bankIban}</span></p>}
            </div>
          </div>
        )}
      </div>

      {/* ═══ TEACHING BREAKDOWN TABLE ═══ */}
      {students.length > 0 && (
        <div style={{ padding: '8px 48px 16px' }}>
          <p style={sectionTitle}>Teaching Breakdown</p>
          <table style={{ width: '100%', fontSize: 12, borderCollapse: 'collapse', marginTop: 8 }}>
            <thead>
              <tr style={{ borderBottom: '2px solid #0a192f' }}>
                <th style={thStyle}>Student</th>
                <th style={thStyle}>Assignment Period</th>
                <th style={{ ...thStyle, textAlign: 'right' }}>Monthly Rate</th>
                <th style={{ ...thStyle, textAlign: 'right' }}>Prorated Days</th>
                <th style={{ ...thStyle, textAlign: 'right' }}>Calculated</th>
                <th style={{ ...thStyle, textAlign: 'right' }}>Final Amount</th>
              </tr>
            </thead>
            <tbody>
              {students.map((s, idx) => {
                const finalAmt = s.editedAmount ?? s.calculatedAmount;
                const hasAdjustment = s.editedAmount !== null && s.editedAmount !== s.calculatedAmount;
                return (
                  <tr key={idx} style={{ borderBottom: '1px solid #f3f4f6' }}>
                    <td style={{ padding: '10px 0' }}>
                      <p style={{ fontWeight: 600, margin: 0 }}>{s.studentName}</p>
                      <p style={{ fontSize: 10, color: '#9ca3af', margin: '1px 0 0', textTransform: 'capitalize' }}>{s.payoutType}</p>
                    </td>
                    <td style={{ padding: '10px 0', color: '#6b7280' }}>
                      {format(parseISO(s.dateFrom), 'dd MMM')} – {format(parseISO(s.dateTo), 'dd MMM')}
                    </td>
                    <td style={{ padding: '10px 0', textAlign: 'right', ...mono }}>
                      {formatAmount(s.payoutRate)}
                    </td>
                    <td style={{ padding: '10px 0', textAlign: 'right', color: '#6b7280' }}>
                      {s.eligibleDays}/{s.totalDays}
                    </td>
                    <td style={{ padding: '10px 0', textAlign: 'right', ...mono, color: '#6b7280' }}>
                      {formatAmount(s.calculatedAmount)}
                    </td>
                    <td style={{ padding: '10px 0', textAlign: 'right', ...mono, fontWeight: 700, color: hasAdjustment ? '#d97706' : undefined }}>
                      {formatAmount(finalAmt)}
                    </td>
                  </tr>
                );
              })}
            </tbody>
          </table>
        </div>
      )}

      {/* ═══ ROLE-BASED SALARY TABLE ═══ */}
      {roleSalaries.length > 0 && (
        <div style={{ padding: '0 48px 16px' }}>
          <p style={sectionTitle}>Role-Based Salary</p>
          <table style={{ width: '100%', fontSize: 12, borderCollapse: 'collapse', marginTop: 8 }}>
            <thead>
              <tr style={{ borderBottom: '2px solid #0a192f' }}>
                <th style={thStyle}>Role</th>
                <th style={thStyle}>Effective Period</th>
                <th style={{ ...thStyle, textAlign: 'right' }}>Monthly Amount</th>
                <th style={{ ...thStyle, textAlign: 'right' }}>Active Days</th>
                <th style={{ ...thStyle, textAlign: 'right' }}>Prorated</th>
                <th style={{ ...thStyle, textAlign: 'right' }}>Final Amount</th>
              </tr>
            </thead>
            <tbody>
              {roleSalaries.map((rs, idx) => {
                const finalAmt = rs.editedAmount ?? rs.proratedAmount;
                return (
                  <tr key={idx} style={{ borderBottom: '1px solid #f3f4f6', background: '#faf5ff' }}>
                    <td style={{ padding: '10px 0' }}>
                      <p style={{ fontWeight: 600, margin: 0 }}>{rs.role.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}</p>
                    </td>
                    <td style={{ padding: '10px 0', color: '#6b7280' }}>
                      {format(parseISO(rs.effectiveFrom), 'dd MMM')} – {format(parseISO(rs.effectiveTo), 'dd MMM')}
                    </td>
                    <td style={{ padding: '10px 0', textAlign: 'right', ...mono }}>{formatAmount(rs.monthlyAmount)}</td>
                    <td style={{ padding: '10px 0', textAlign: 'right', color: '#6b7280' }}>{rs.activeDays}/{rs.totalDays}</td>
                    <td style={{ padding: '10px 0', textAlign: 'right', ...mono, color: '#6b7280' }}>{formatAmount(rs.proratedAmount)}</td>
                    <td style={{ padding: '10px 0', textAlign: 'right', ...mono, fontWeight: 700 }}>{formatAmount(finalAmt)}</td>
                  </tr>
                );
              })}
            </tbody>
          </table>
        </div>
      )}

      {/* ═══ SALARY CALCULATION SECTION ═══ */}
      <div style={{ padding: '0 48px 24px' }}>
        <div style={{ background: '#f8fafc', borderRadius: 12, padding: 24, border: '1px solid #e2e8f0' }}>
          {/* EARNINGS */}
          <div style={{ marginBottom: 16 }}>
            <p style={{ fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.12em', color: '#059669', fontWeight: 700, marginBottom: 8 }}>Earnings</p>
            <SummaryRow label="Base Salary" amount={baseSalary} />
            {extraClassAmount > 0 && <SummaryRow label="Extra Classes" amount={extraClassAmount} color="#059669" prefix="+" />}
            {additionAdjs.map((adj, idx) => (
              <SummaryRow
                key={idx}
                label={`${adjustmentLabel(adj.adjustment_type)}${adj.adjustment_mode === 'percentage' && adj.percentage_value ? ` (${adj.percentage_value}%)` : ''}${adj.reason ? ` — ${adj.reason}` : ''}`}
                amount={adj.resolved_amount ?? adj.amount}
                color="#059669"
                prefix="+"
              />
            ))}
          </div>

          {/* DEDUCTIONS */}
          {(deductions > 0 || deductionAdjs.length > 0) && (
            <div style={{ marginBottom: 16 }}>
              <p style={{ fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.12em', color: '#dc2626', fontWeight: 700, marginBottom: 8 }}>Deductions</p>
              {deductionAdjs.map((adj, idx) => (
                <SummaryRow
                  key={idx}
                  label={`${adjustmentLabel(adj.adjustment_type)}${adj.adjustment_mode === 'percentage' && adj.percentage_value ? ` (${adj.percentage_value}%)` : ''}${adj.reason ? ` — ${adj.reason}` : ''}`}
                  amount={adj.resolved_amount ?? adj.amount}
                  color="#dc2626"
                  prefix="-"
                />
              ))}
              {deductions > 0 && deductionAdjs.length === 0 && (
                <SummaryRow label="Total Deductions" amount={deductions} color="#dc2626" prefix="-" />
              )}
            </div>
          )}

          {/* SUMMARY */}
          <div style={{ borderTop: '2px solid #0a192f', paddingTop: 12, marginTop: 8 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 6 }}>
              <span style={{ color: '#6b7280' }}>Gross Salary</span>
              <span style={mono}>PKR {formatAmount(baseSalary + extraClassAmount + additions)}</span>
            </div>
            {deductions > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 6, color: '#dc2626' }}>
                <span>Total Deductions</span>
                <span style={mono}>-PKR {formatAmount(deductions)}</span>
              </div>
            )}
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 20, fontWeight: 800, paddingTop: 8, borderTop: '1px dashed #d1d5db' }}>
              <span>Net Salary</span>
              <span style={{ ...mono, color: '#0a192f' }}>PKR {formatAmount(netSalary)}</span>
            </div>
          </div>
        </div>
      </div>

      {/* ═══ PAYMENT DETAILS ═══ */}
      {(paymentDate || paymentMethod || receiptUrl) && (
        <div style={{ padding: '0 48px 20px' }}>
          <p style={sectionTitle}>Payment Details</p>
          <div style={{ display: 'flex', gap: 32, fontSize: 13, marginTop: 8 }}>
            {paymentDate && <div><span style={{ color: '#6b7280' }}>Payment Date:</span> <strong>{paymentDate}</strong></div>}
            {paymentMethod && <div><span style={{ color: '#6b7280' }}>Method:</span> <strong>{paymentMethod}</strong></div>}
          </div>
          {receiptUrl && /\.(jpg|jpeg|png|webp|gif)(\?|$)/i.test(receiptUrl) && (
            <div style={{ marginTop: 10, border: '1px solid #e5e7eb', borderRadius: 8, padding: 8, display: 'inline-block' }}>
              <img src={receiptUrl} alt="Payment proof" style={{ maxHeight: 120, borderRadius: 4 }} />
            </div>
          )}
        </div>
      )}

      {/* Signatures */}
      <div style={{ padding: '0 48px 20px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, marginTop: 16 }}>
          <div style={{ borderTop: '1px solid #d1d5db', paddingTop: 8 }}>
            <p style={{ fontSize: 10, color: '#9ca3af', margin: 0 }}>Prepared By</p>
          </div>
          <div style={{ borderTop: '1px solid #d1d5db', paddingTop: 8 }}>
            <p style={{ fontSize: 10, color: '#9ca3af', margin: 0 }}>Received By</p>
          </div>
        </div>
      </div>

      {/* Footer */}
      <div style={{ padding: '14px 48px', borderTop: '1px solid #e5e7eb', textAlign: 'center', background: '#fafbfc' }}>
        <p style={{ fontSize: 9, color: '#9ca3af', margin: 0 }}>This is a system-generated salary statement.</p>
        <p style={{ fontSize: 9, color: '#9ca3af', margin: '2px 0 0' }}>{orgName} • Generated on {format(new Date(), 'dd MMM yyyy')}</p>
      </div>

      <div style={{ height: 4, background: 'linear-gradient(90deg, #0a192f 0%, #00a8e8 50%, #0a192f 100%)' }} />
    </div>
  );
}

// ── Helper components ──

const thStyle: React.CSSProperties = {
  textAlign: 'left',
  padding: '8px 0',
  fontSize: 9,
  textTransform: 'uppercase',
  letterSpacing: '0.12em',
  color: '#6b7280',
  fontWeight: 700,
};

function SummaryRow({ label, amount, color, prefix }: { label: string; amount: number; color?: string; prefix?: string }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 4, color: color || undefined }}>
      <span style={{ maxWidth: '70%' }}>{label}</span>
      <span style={{ fontFamily: "'JetBrains Mono', monospace" }}>{prefix || ''}PKR {amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
    </div>
  );
}

function adjustmentLabel(type: string) {
  switch (type) {
    case 'bonus': return 'Bonus';
    case 'allowance': return 'Allowance';
    case 'deduction': return 'Deduction';
    case 'expense': return 'Expense Reimbursement';
    case 'loan': return 'Loan';
    case 'penalty': return 'Penalty';
    default: return type;
  }
}

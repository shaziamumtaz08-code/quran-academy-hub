...
export function TeachTodayTab({ courseId }: TeachTodayTabProps) {
  ...
  // First get syllabus for this course
  ...
  // Fetch session plans for the syllabus
  ...
  const activePlan = selectedPlanId
    ? sessionPlans.find((p: any) => p.id === selectedPlanId)
    : sessionPlans[0];
  ...
  const { data: contentKit, isLoading: kitLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .limit(1);
      const kit = data?.[0] || null;
      if (!kit) return null;
      // Fetch new columns separately to avoid type issues
  ...
  const activities: Array<{ title: string; duration: number; type?: string }> = activePlan?.activities
    ? (typeof activePlan.activities === 'string' ? JSON.parse(activePlan.activities) : activePlan.activities)
    : [];
          ...
          <Select value={selectedPlanId || activePlan?.id || ''} onValueChange={setSelectedPlanId}>
            ...
            <SelectContent>
              {sessionPlans.map((p: any) => (
                <SelectItem key={p.id} value={p.id}>{p.session_title || 'Untitled plan'}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        )}
            ...
            <CardContent className="p-4 space-y-3">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h3 className="font-semibold text-sm">{activePlan.session_title || 'Untitled'}</h3>
                </div>
                {activePlan.session_objective && (
                  <p className="text-xs text-muted-foreground">{activePlan.session_objective}</p>
                )}
              </div>
